...
export default function ResidencePage({ params }: ResidencePageProps) {
  ...
  const [mobileCalendarMonths, setMobileCalendarMonths] = useState(36)
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // 스크롤이 하단 근처에 도달하면 더 많은 달 로드
      const scrollTop = container.scrollTop
      const scrollHeight = container.scrollHeight
      const clientHeight = container.clientHeight
      const scrollPercentage = (scrollTop + clientHeight) / scrollHeight

      if (scrollPercentage > 0.3 && mobileCalendarMonths < 120) {
        clearTimeout(scrollTimeout)
        scrollTimeout = setTimeout(() => {
          setMobileCalendarMonths(prev => Math.min(prev + 30, 120))
        }, 100)
      }
    }
        ...
        <div className="fixed inset-0 z-[9999] bg-[#FFF] lg:hidden flex flex-col">
          ...
          <div className="flex items-center justify-between p-4 border-b border-[#e9eaec] bg-white flex-shrink-0">
            <h3 className="text-[18px] font-bold text-[#14151a]">
              {isCheckInCalendarOpen ? 'Check-in' : 'Check-out'}
            </h3>
            <button
              onClick={() => {
                setIsCheckInCalendarOpen(false)
          ...
          >
            ...
            `}} />
            <DatePicker
              selected={isCheckInCalendarOpen ? checkInDate : checkOutDate}
              onChange={(date) => {
                if (isCheckInCalendarOpen) {
                  handleCheckInChange(date)
                  if (date) {
                    setIsCheckInCalendarOpen(false)
                  }
              ...
              inline
              locale={datePickerLocale}
              minDate={isCheckInCalendarOpen ? new Date() : (checkInDate ? new Date(checkInDate.getFullYear(), checkInDate.getMonth(), checkInDate.getDate()) : new Date())}
              monthsShown={mobileCalendarMonths}
              calendarClassName="!border-none !w-full"
            />
          </div>
        </div>
